import { createCar, createWinner, getCar, getCars, getWinner, getWinners, removeCar, removeWinner, startEngine, updateCar, updateWinner } from "./api";
import { renderCarImage } from "./carImage";
import { carType, newCarType, winnerType } from "./types";
import { animateCar, generateRandomCars, race, returnCarToBase } from "./utils";
...
export const renderGarage = async () => {
    ...
    return ` ${cars.length ? `<h2>Garage ${count}</h2>
    <h3>Page: ${page}</h3>
    ...
    return (`
        <div class="winner-container">
            ...
            <div class="table-buttons">
                <button class="prev-table" ${(tablePage === 1 || count === 0) ? "disabled" : ""}>Prev</button>
                <button class="next-table" ${pageCount === page ? "disabled" : ""}>Next</button>
            </div>

            ${winnersList.length ? `<table>
            <thead>
                <tr>
                    <th class="id-sort">№</th>
                    <th>Car</th>
            ...
            ${
                cars.map((car) => {
                    ...
                    return(`
                        <tr>
                            <td>${car.id}</td>
                            <td>${renderCarImage(car.color)}</td>
                            <td>${car.name}</td>
                            <td>${winnersList[findIndex].wins}</td>
                            <td>${winnersList[findIndex].time/1000}s</td>
                        </tr>
                    `);
                }).join('')
            }
...
export const listen = () => {
    document.body.addEventListener('click', async (event: Event) => {
        ...
        if((event.target as HTMLElement).classList.contains('car-select-btn')) {
            event.preventDefault();
            const id: number = +(event.target as HTMLElement).id.split('select-car-')[1];
            selectedId = id;
            const car = getCar(id);
            ...
            (document.querySelector('.update-btn') as HTMLButtonElement).disabled = false;
            (document.querySelector('#update-name') as HTMLInputElement).value = (await car).name;
            (document.querySelector('#update-color') as HTMLInputElement).value = (await car).color;
        }
