...
export const renderWinner = async () => {
  ...
  const winnersList: winnerType[] = await (
  ...
  document.body.addEventListener('click', async (event: Event) => {
    ...
    if ((event.target as HTMLElement).classList.contains('create-btn')) {
      ...
      const createNameValue = (
        document.querySelector('#create-name') as HTMLInputElement
      ).value;
      const createColorValue = (
        document.querySelector('#create-color') as HTMLInputElement
      ).value;
    ...
    if ((event.target as HTMLElement).classList.contains('update-btn')) {
      ...
      if (selectedId !== null) {
        ...
        const updateColorValue = (
          document.querySelector('#update-color') as HTMLInputElement
        ).value;
        const newCar: newCarType = {
    ...
    if (
      (event.target as HTMLButtonElement).classList.contains('generate-btn')
    ) {
